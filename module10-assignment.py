...
df_missing_handled["satisfaction_rating"] = (
    df_missing_handled["satisfaction_rating"]
...
date_fill_strategy = "forward_fill" # Uses forward fill for last_purchase because the data goes in order by customer purchase, so using the last purchase for missing values is best
df_missing_handled["last_purchase"] = (
    df_missing_handled["last_purchase"]
    .ffill()
)
...
df_missing_handled["phone"] = df_missing_handled["phone"].fillna("Unknown")
df_missing_handled["loyalty_status"] = df_missing_handled["loyalty_status"].fillna("Bronze") # Fills the missing loyalty_status with Bronze because it is the lowest status
df_missing_handled["age"] = df_missing_handled["age"].fillna(df_missing_handled["age"].median())
...
df_typed["total_purchases"] = df_typed["total_purchases"].fillna(0) # Fill missing values before converting to integers 
...
df_no_duplicates["average_purchase_value"] = (
    ...
    df_no_duplicates["total_purchases"].replace(0, np.nan)
)
...
df_no_duplicates["purchase_frequency_category"] = df_no_duplicates["total_purchases"].map(purchase_frequency)
...
df_renamed = df_no_duplicates.rename(columns={
    ...
    "purchase_frequency_category": "PurchaseFrequencyCategory"
})
...
final_missing_check = df_final.isnull().sum()
...
satisfaction_spend_corr = float(df_final["SatisfactionRating"].corr(df_final["TotalSpent"]))
...
print(f" - Data Type Issues: join_date and last_purchase were strings, total_spent included commas and dollar signs, and numeric fields needed conversion")
...
print("- Categories: Standardized to title case so the capitalization is consistent")
print(f"- Phone Numbers: Standardized to (###) ###-#### format")
